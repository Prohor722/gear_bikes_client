...
const Reviews = () => {
  ...
  // const {data:reviewCount} = useQuery('pages',()=>fetch('http://localhost:5000/reviewCount').then(res=>res.json()));

  const { data: reviews, isLoading } = useQuery(["reviews", currentPage], () =>
    fetch(`http://localhost:5000/reviews?page=${currentPage}`).then((res) =>
      res.json()
    )
  );
